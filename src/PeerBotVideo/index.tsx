...
export const PeerBotVideo: FC = () => {
  const { host_user_id: hostUserId } = useParams();
  if (!hostUserId) throw new Error('host_user_id is required');
  const [searchParams] = useSearchParams();
  const currentUserId = searchParams.get('user_id');
  if (!currentUserId) throw new Error('user_id search param is required');
  ...
  useEffect(() => {
    ...
    const createPeer = () => {
      const peer = new Peer(getPeerId(PAGE_PREFIX, currentUserId), {
  ...
  }, [currentUserId, mediaStream, handleNewConnection]);
  ...
  useEffect(() => {
    ...
    const connectionId = getPeerId(PAGE_PREFIX, hostUserId);
  ...
  }, [peer, mediaStream, hostUserId, currentUserId, handleNewConnection, isOtherUserConnected]);
  ...
  return (
    <Stack direction="column" flexGrow={1} gap={2} height="100%" position="relative">
      ...
      {!isOtherUserConnected && currentUserId === hostUserId && (
        <Stack
          direction="column"
          flexGrow={1}
          gap={4}
